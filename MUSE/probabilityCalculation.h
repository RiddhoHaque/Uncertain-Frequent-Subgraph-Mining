...
pair <double, double> evaluate_exact(vector <Embedding> embeddings){
    ...
    for(int i=1; i<=(int)embeddings.size(); i++){
        ...
        while(subset.size()>0){
            ...
            if(caching) probability[subset]=val;
            if(i%2) sum+=val;
            else sum-=val;
            subset=getNextSubset(subset, i);
        }
    }
    if(caching) probability.clear();
    return {sum, sum};
}
...
pair <double, double> evaluate_approx(vector <Embedding> embeddings){
    ...
    double eps_prime=EPS*m_sup/2.0;
    double rounds=(4.0*(int)embeddings.size()*log(2.0/LAMBDA))/(eps_prime*eps_prime);
...
pair <double, double> evaluate(vector <Embedding> embeddings){
    ...
    for(auto jointEmbedding: partitions){
        ...
        if(flag) break;
        maxSize=max(maxSize, (int) jointEmbedding.size());
    }
    if(flag){
        //cerr<<"evaluating approx."<<endl;
